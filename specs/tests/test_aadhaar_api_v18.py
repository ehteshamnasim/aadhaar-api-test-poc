...
def test_get_demographics_success(client):
    ...
    assert response.status_code == 200
    assert 'demographic_data' in response.json


# 🔄 REGENERATED - Endpoint modified
def test_get_demographics_missing_aadhaar_number(client):
    """
    Test error handling when 'aadhaar_number' is missing from the payload.
    """
...
def test_get_demographics_missing_consent(client):
    """
    Test error handling when 'consent' is missing from the payload.
    """
...
def test_get_demographics_invalid_aadhaar_number(client):
    """
    Test error handling with an invalid 'aadhaar_number'.
    """
...
def test_get_demographics_invalid_consent(client):
    """
    Test error handling with an invalid 'consent' value.
    """
    response = client.post('/api/v1/aadhaar/demographics', json={"aadhaar_number": "123456789012", "consent": "yes"})
    assert response.status_code == 400
    assert 'error' in response.json
