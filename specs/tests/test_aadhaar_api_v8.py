...
def test_verify_aadhaar_with_rate_limit_exceeded(client):
    ...
    """
    # Simulate sending multiple requests to exceed rate limit
    for _ in range(10):  # Assuming the rate limit is 5 requests per minute
        payload = {
            "aadhaar_number": "123456789012",
            "captcha": "abc123"
        }
...
# 🔄 REGENERATED - Endpoint modified
def test_verify_aadhaar_with_forbidden_access(client):
    """
    Test verifying Aadhaar number with forbidden access.
    Expected response: 403 Forbidden
    """
    # This test assumes there is some condition that leads to a 403 response
    payload = {
        "aadhaar_number": "123456789012",
        "captcha": "abc123"
    }
