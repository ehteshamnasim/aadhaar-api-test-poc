"""AI-Generated Tests v41 - 2025-10-28 02:48:24"""
import pytest
...
def test_demographics_invalid_consent(client):
...
def test_ekyc_invalid_consent(client):
    response = client.post('/api/v1/aadhaar/ekyc', json={'aadhaar_number': '123456789012', 'transaction_id': '123456789', 'consent': 'invalid'})
    assert response.status_code == 400


def test_ekyc_missing_field(client):
    response = client.post('/api/v1/aadhaar/ekyc', json={'aadhaar_number': '123456789012', 'transaction_id': '123456789'})
    assert response.status_code == 400


def test_ekyc_success(client):
    response = client.post('/api/v1/aadhaar/ekyc', json={'aadhaar_number': '123456789012', 'transaction_id': '123456789', 'consent': True})
    assert response.status_code == 200
...
def test_verify_otp_success(client):
    response = client.post('/api/v1/aadhaar/otp/verify', json={'transaction_id': '123456789', 'otp': '123456'})
    assert response.status_code == 200
